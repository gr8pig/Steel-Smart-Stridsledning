  ...
  for (let i = 0; i < count; i++) {
    ...
    newTracks.push({
      ...
      intentDistribution: isFeint
        ? { probe: 0.10, feint: conf, strike: 0.05, saturation: 0.10, decoy: 1 - conf - 0.25 }
        : { probe: 0.02, feint: 0.03, strike: conf, saturation: 1 - conf - 0.05, decoy: 0.02 },
...
app.get('/api/twins/campaign', (_req, res) => {
  res.json({
    ...
    threats: threats.filter(t => t.status !== 'NEUTRALIZED' && t.status !== 'LEAKED'),
    policy: {
      id: 'POL-01', name: 'Standard Defense Posture',
...
app.post('/api/rationale/coa', async (req, res) => {
  ...
  const system =
    'You are the AI tactical advisor for Steel - Smart Stridsledning, a NATO-standard air defense ' +
...
app.post('/api/rationale/lab-result', async (req, res) => {
  ...
  const system =
    'You are an operations research analyst briefing a Nordic air defense commander. ' +
...
app.post('/api/rationale/logistics', async (req, res) => {
  ...
  const system =
    'You are a theater logistics AI for a Nordic air defense operation. ' +
