...
export const REFERENCE_DOCS: ReferenceDoc[] = [
  ...
  makeDoc({
    ...
    sections: [
      ...
      },
      {
        ...
        summary: 'Several features are present but still rely on local heuristics or seed data for their edge cases.',
        bullets: [
          'Public capability layer is implemented as a live remapping switch over synthetic tracks and public-source cards.',
          'Knowledge graph situation awareness is now a first-class platform map with live-injected node data.',
          'Robustness lab and counterfactual flows are fully productionized via the Python FastAPI backend.',
          'Counterfactual lab calls the ensemble ML endpoint (`/api/ml/predict`) and RunPod orchestration (`/api/ml/deep-sim`).',
          'C2 resilience uses authoritative server-side scoring from the FastAPI decision fabric module.',
          'Solver, rationale, and sensor/feed seams are fully wired to the Python simulation engine.',
        ],
      },
      {
  ...
  makeDoc({
    ...
    status: 'implemented',
    summary: 'The platform architecture is a dual-tier system where an Angular SSR frontend is paired with an authoritative Python FastAPI backend for all domain logic and simulation.',
    graph: { x: 530, y: 270, links: ['01-product-overview', '03-information-architecture', '05-domain-models', '10-deployment-operations'] },
    sources: [
      { label: 'API Backend', path: 'api/main.py', note: 'FastAPI entry point and authoritative route registry.' },
      { label: 'Twin Engine', path: 'api/twin_engine.py', note: 'Authoritative state evolution and campaign twin model.' },
      { label: 'Decision Fabric', path: 'api/decision_fabric.py', note: 'Server-side resilience scoring and friction mechanics.' },
      { label: 'Node Edge', path: 'src/server.ts', note: 'Authenticated Express proxy and SSR shell.' },
      { label: 'Steel API', path: 'src/app/core/services/steel-api.service.ts', note: 'Canonical client-to-backend communication bridge.' },
      { label: 'ML Inference', path: 'api/ml/inference.py', note: 'Fast-path Ensemble LightGBM quantile regression.' },
      { label: 'RunPod Client', path: 'api/ml/runpod_client.py', note: 'Orchestrator for deep-sim GPU worker jobs.' },
      { label: 'Replay Registry', path: 'api/replay.py', note: 'Idempotent deduplication for synced operational actions.' },
    ],
    sections: [
      {
      ...
      },
      {
        ...
        summary: 'Unlike traditional prototypes, domain truth in Steel is owned by the Python backend, not the browser.',
        bullets: [
          'FastAPI owns resilience scoring, intent estimation, and COA selection.',
          'The Python twin engine is the single source of truth for the theater state.',
          'The Angular client functions as a high-fidelity display and command interface.',
          'This split ensures that complex simulation logic runs in a performant, scalable environment.',
        ],
      },
    ],
