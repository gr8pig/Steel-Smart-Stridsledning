...
@Component({
  ...
  imports: [
    CommonModule,
    RouterLink,
    KnowledgeGraphViewerComponent,
    MatIconModule,
  ],
  changeDetection: ChangeDetectionStrategy.OnPush,
  template: `
    <div
      class="flex h-full w-full flex-col overflow-hidden bg-[#05080d] text-[#E2E8F0]"
    >
      <header
        class="flex items-center justify-between gap-4 border-b border-white/10 bg-[#080c12]/95 px-6 py-4 backdrop-blur z-30 shrink-0"
      >
        <div class="min-w-0">
          <div
            class="text-[9px] font-black uppercase tracking-[0.35em] text-sky-300/80"
          >
            Boreal Info_Arch
          </div>
          ...
          <p class="mt-1 max-w-3xl text-[11px] leading-relaxed text-slate-400">
            A 3D WebGL map of runtime UI, state, services, backend seams, docs,
            research, and scaffold assets.
          </p>
        </div>

        <div class="flex items-center gap-2">
          <label
            class="hidden items-center gap-2 rounded-sm border border-white/10 bg-white/5 px-3 py-2 text-[10px] font-mono uppercase tracking-[0.18em] text-slate-400 md:flex"
          >
      ...
      <section
        class="flex min-h-0 flex-1 flex-col overflow-hidden md:flex-row relative"
      >
        <div
          class="relative min-w-0 flex-1 h-full w-full bg-black overflow-hidden border-r border-white/10"
        >
          <div
            class="absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(96,165,250,0.12),transparent_32%),radial-gradient(circle_at_bottom_right,rgba(16,185,129,0.10),transparent_28%),linear-gradient(180deg,rgba(5,8,13,0.86),rgba(5,8,13,1))] pointer-events-none z-0"
          ></div>
          ...
          <div
            class="absolute bottom-4 left-4 z-20 flex items-center gap-3 text-[10px] font-mono uppercase tracking-[0.22em] text-slate-400 pointer-events-none"
          >
            <span
        ...
        @if (selectedNode(); as node) {
          <aside
            class="absolute top-0 right-0 bottom-0 z-40 flex w-full flex-col border-l border-white/10 bg-[#080b11]/95 backdrop-blur-md md:w-[450px] shadow-2xl"
          >
            <div class="flex h-full min-h-0 flex-col overflow-hidden">
              <div
                class="border-b border-white/10 px-6 py-6 flex justify-between items-start"
              >
                <div>
                  <div
                    class="text-[9px] font-black uppercase tracking-[0.32em] text-sky-300/80"
                  >
                    Node Details
                  </div>
                  <h2
              ...
              <div class="min-h-0 flex-1 overflow-y-auto px-6 py-5">
                <div class="grid gap-3 sm:grid-cols-2">
                  <div class="rounded-sm border border-white/10 bg-white/5 p-3">
                    <div
                      class="text-[8px] font-black uppercase tracking-[0.24em] text-slate-500"
                    >
                      Status
                    </div>
                    <div
                      class="mt-1 text-[11px] font-bold uppercase tracking-[0.18em] text-white"
                    >
                      {{ node.status }}
                    </div>
                  </div>
                  <div class="rounded-sm border border-white/10 bg-white/5 p-3">
                    ...
                    <div
                      class="mt-1 text-[11px] font-bold uppercase tracking-[0.18em] text-sky-300"
                    >
                      {{ node.area }}
                    </div>
                  </div>
                ...
                <div class="mt-4 space-y-4">
                  <div class="rounded-sm border border-white/10 bg-white/5 p-4">
                    ...
                    <p class="mt-2 text-[12px] leading-relaxed text-slate-200">
                      {{ node.what }}
                    </p>
                  </div>
                  <div class="rounded-sm border border-white/10 bg-white/5 p-4">
                ...
                <div
                  class="mt-4 rounded-sm border border-white/10 bg-white/5 p-4"
                >
                  <div
                    class="text-[8px] font-black uppercase tracking-[0.24em] text-slate-500"
                  >
                ...
                >
                  ...
                  <code class="mt-2 block break-all text-[10px] text-sky-200">{{
                    node.sourcePath ?? "No explicit source path"
                  }}</code>
                </div>
                  ...
                  <div class="mt-3 grid gap-3">
                    <div>
                      ...
                      <div class="mt-2 flex flex-wrap gap-2">
                        @for (
                          input of node.technicalSpecs.inputs;
                          track input
                        ) {
                          <span
                            class="rounded-sm border border-sky-400/20 bg-sky-400/10 px-2 py-1 text-[9px] font-bold uppercase tracking-[0.16em] text-sky-200"
                            >{{ input }}</span
                          >
                        }
                      </div>
                    ...
                    <div>
                      ...
                      <div class="mt-2 flex flex-wrap gap-2">
                        @for (
                          output of node.technicalSpecs.outputs;
                          track output
                        ) {
                          <span
                            class="rounded-sm border border-emerald-400/20 bg-emerald-400/10 px-2 py-1 text-[9px] font-bold uppercase tracking-[0.16em] text-emerald-200"
                            >{{ output }}</span
                          >
                        }
                      </div>
                    ...
                    @if (node.technicalSpecs.logic) {
                      <div>
                        ...
                        <p
                          class="mt-2 text-[11px] leading-relaxed text-slate-300"
                        >
                    ...
                    @if (node.technicalSpecs.math) {
                      <div>
                        ...
                        <p
                          class="mt-2 whitespace-pre-line font-mono text-[10px] leading-relaxed text-slate-300"
                        >
                    ...
                    @if (node.technicalSpecs.uncertaintySource) {
                      <div>
                        ...
                        <p
                          class="mt-2 text-[11px] leading-relaxed text-slate-300"
                        >
                          {{ node.technicalSpecs.uncertaintySource }}
                        </p>
                      </div>
                    }
                    @if (node.technicalSpecs.fatiguePenalty) {
                      <div>
                        ...
                        <p
                          class="mt-2 text-[11px] leading-relaxed text-slate-300"
                        >
                          {{ node.technicalSpecs.fatiguePenalty }}
                        </p>
                      </div>
                    }
                    @if (node.technicalSpecs.policyDriftOffset) {
                      <div>
                        ...
                        <p
                          class="mt-2 text-[11px] leading-relaxed text-slate-300"
                        >
                          {{ node.technicalSpecs.policyDriftOffset }}
                        </p>
                      </div>
                    }
                  </div>
                ...
                <div
                  class="mt-4 rounded-sm border border-white/10 bg-white/5 p-4 mb-8"
                >
                  <div
                    class="text-[8px] font-black uppercase tracking-[0.24em] text-slate-500"
                  >
...
export class KnowledgeGraph {
  ...
  readonly twinNode = computed(() => {
    return (
      this.selectedNode() ??
      this.nodes().find((node) => node.id === "PLAT_001") ??
      null
    );
  });
  ...
  readonly filteredNodeIds = computed(
    () => new Set(this.visibleNodes().map((node) => node.id)),
  );
  ...
  readonly graphLines = computed(() => {
    ...
    for (const source of nodes) {
      for (const targetId of source.connectedTo) {
        ...
        const isFocusedPair = focus
          ? relatedIds.has(source.id) && relatedIds.has(targetId)
          : false;
        const isFilteredPair =
          filteredIds.has(source.id) && filteredIds.has(targetId);
        ...
        lines.push({
          ...
          active: focus
            ? isFocusedPair ||
              filteredIds.has(source.id) ||
              filteredIds.has(targetId)
            : isFilteredPair || filteredIds.size === nodes.length,
          highlighted: focus
            ? source.id === focus.id || targetId === focus.id
            : isFilteredPair,
        });
      }
    }
  ...
  isNodeDimmed(nodeId: string): boolean {
    ...
    if (
      selected &&
      !this.relatedNodeIds().has(nodeId) &&
      this.viewMode() === "GRAPH"
    ) {
      return true;
    }
